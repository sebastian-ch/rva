...
| 2 | LoD2 roofs from the 2025 LiDAR (`roofer`) | **done 2026-09-14** — 22,164 citywide roofs imported with measured quality gates and procedural fallback |
...
After geometry validation and source-ID joining, 22,164 of the map's 25,533 building records use the
measured roof; the remaining 3,369 retain the existing roof. Detailed meshes are rendered only in the
...
worker p95 remained 28 ms. This gate lives in `pipeline/lod2.py` and should be recalibrated for new data.
