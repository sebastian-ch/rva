...
export function decodePoiTable(data: ArrayBuffer, bbox: [number, number, number, number]): FC<PointGeom, PoiProps> {
  ...
  const features = Array.from({ length: count }, (_, i) => {
    ...
    // Trees store height and crown radius; every other kind stores a pole height and a heading (projected radians).
    const height = view.getFloat32(offset + 24, true), second = view.getFloat32(offset + 28, true);
    ...
    if (Number.isFinite(second)) props[kind === 'tree' ? 'crown_radius' : 'heading'] = second;
