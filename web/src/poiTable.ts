...
const KINDS = ['', 'tree', 'streetlight', 'bench', 'bus_stop', 'traffic_signals', 'fountain', 'monument', 'shop', 'restaurant', 'museum', 'lamp_post', 'utility_pole'];
...
export function decodePoiTable(data: ArrayBuffer, bbox: [number, number, number, number]): FC<PointGeom, PoiProps> {
  ...
  const features = Array.from({ length: count }, (_, i) => {
    ...
    // The first float is the tree height for trees and the pole height for surveyed lamps and poles.
    const height = view.getFloat32(offset + 24, true), crownRadius = view.getFloat32(offset + 28, true);
    ...
    if (Number.isFinite(height)) props[kind === 'tree' ? 'tree_height' : 'pole_height'] = height;
