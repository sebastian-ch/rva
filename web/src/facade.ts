...
const PARKING = new Set(['garage', 'garages', 'parking']);
...
export function facadeParams(p: BuildingProps): FacadeParams {
  ...
  let style: number;
  if (PARKING.has(t)) style = STYLE_PARKING;
  else if (RETAIL.has(t)) style = STYLE_RETAIL;
  ...
  let floor = style === STYLE_OFFICE ? 3.6 : style === STYLE_PARKING ? 3.0 : 3.2;
